...
A command line Java application made by Group 10 for **MotorPH's Payroll Module** that presents employee information and calculates salaries with government-mandated deductions, built as a single `.java` file with no OOP concepts and no external libraries.
...
├── employees.csv             ← Required at runtime (not tracked in git — see setup)
├── attendance.csv            ← Required at runtime (not tracked in git — see setup)
└── README.md
```

> Employee data is **read from `employees.csv`** at runtime (exported from the "Employee Details" sheet).  
> Attendance data is **read from `attendance.csv`** at runtime (exported from the "Attendance Record" sheet).  
> Neither CSV file is committed to the repository — both must be exported from the Excel file before running.
...
### Step 1 — Export Employee CSV

1. Open **`Copy_of_MotorPH_Employee_Data.xlsx`**
2. Click the **`Employee Details`** sheet tab at the bottom
3. Go to **File → Save As → CSV (Comma delimited) (.csv)**
4. Save the file as **`employees.csv`**
5. Place `employees.csv` in the **same folder** as `MotorPHPayroll.java`

> The CSV must have these columns in this order (header row is automatically skipped):  
...
1. In the same Excel file, click the **`Attendance Record`** sheet tab
2. Go to **File → Save As → CSV (Comma delimited) (.csv)**
3. Save the file as **`attendance.csv`**
4. Place `attendance.csv` in the **same folder** as `MotorPHPayroll.java`
...
2. Place both `employees.csv` and `attendance.csv` in the same folder as `MotorPHPayroll.java`
...
Password: 12345
[INFO] Loaded 34 employee records from 'employees.csv'.
...
```
main()
 ├── loadEmployeeCsv()               ← reads employees.csv into memory
...
When the program starts, `main` loads all employee and attendance data from their respective CSV files, then immediately prompts for a username and password.

**Employee data** is loaded from `employees.csv` (exported from the "Employee Details" sheet). **Attendance data** is loaded from `attendance.csv` (exported from the "Attendance Record" sheet). Both loaders skip the header row and strip stray quotes or whitespace from each field automatically.

Valid credentials are `employee`/`12345` and `payroll_staff`/`12345`. If either the username or password is wrong, the program prints `"Incorrect username and/or password."` and terminates immediately — no menu is shown at all.

If login succeeds, the program branches: `employee` → `runEmployeeMenu()`, `payroll_staff` → `runPayrollStaffMenu()`.

---

### 2. `loadEmployeeCsv()` — Reading Employee Data from File

This method reads employee records from `employees.csv` (the "Employee Details" sheet exported as CSV). It works identically to the attendance loader: it skips the header row, parses each line using `parseCsvLine()`, strips whitespace and quotes from every field, and normalizes the Employee # by removing any trailing `.0` (an artifact of Excel's float export).

Only rows with at least 19 columns and a non-empty Employee # are kept. Column indexes are named via constants so any reference to a column in the rest of the program is readable:

| Constant | Index | Field |
|----------|-------|-------|
| `EMP_ID` | 0 | Employee # |
| `EMP_LAST_NAME` | 1 | Last Name |
| `EMP_FIRST_NAME` | 2 | First Name |
| `EMP_BIRTHDAY` | 3 | Birthday |
| `EMP_ADDRESS` | 4 | Address |
| `EMP_PHONE` | 5 | Phone Number |
| `EMP_SSS` | 6 | SSS # |
| `EMP_PHILHEALTH` | 7 | Philhealth # |
| `EMP_TIN` | 8 | TIN # |
| `EMP_PAGIBIG` | 9 | Pag-ibig # |
| `EMP_STATUS` | 10 | Status |
| `EMP_POSITION` | 11 | Position |
| `EMP_SUPERVISOR` | 12 | Immediate Supervisor |
| `EMP_BASIC_SALARY` | 13 | Basic Salary |
| `EMP_RICE` | 14 | Rice Subsidy |
| `EMP_PHONE_ALLOW` | 15 | Phone Allowance |
| `EMP_CLOTHING` | 16 | Clothing Allowance |
| `EMP_SEMI_MONTHLY` | 17 | Gross Semi-monthly Rate |
| `EMP_HOURLY_RATE` | 18 | Hourly Rate (= Basic Salary ÷ 21 ÷ 8) |

If the file is missing, the program prints clear export instructions and exits rather than crashing.
